...
The builder is a toolbox-based agent system that helps researchers create ISA-Tox profile-compliant RO-Crates. It ships **two first-class, LangChain-powered build architectures over the same toolbox** — entity drafting, lookups, validation, assessment: a **deterministic pipeline + HITL guidance tail** (the `--interactive` default — code owns the step ordering and the LLM is confined to bounded leaves), and a **ReAct agent loop** (`--react` — the LLM dynamically decides which tools to call based on the current state of the crate). Both are maintained; this is an ongoing exploration, not a one-way migration.
...
# The pipeline arm is folder-driven: with no --input there is nothing to scan, so
# it prints a notice and exits. To start with no input folder, use --react below.
...
conformance, FAIR maturity, MIT coverage, reproducibility readiness — and an
...
- **Default** (no `--output`, with `--input`): **`output/<name>_crate`**, where
  `<name>` is the input folder's name with a trailing `_extracted` stripped. For
  example, `-i /data/experiment/` writes to `output/experiment_crate`. Re-runs are
  versioned `_v2` / `_v3` …, so a rebuild never overwrites a previous export — and
  builds stay out of a curated input tree.
- No `--input` (conversation mode): the session working directory
  (`sessions/<session_id>/working_crate/`).

```bash
# Default: writes to output/experiment_crate (versioned _v2/_v3… on re-run)
uv run python -m main --interactive -i /path/to/experiment/
...
| ``VITRO_OPENAI_BASE_URL`` | API base URL override | — (unset ⇒ the SDK's own default, ``https://api.openai.com/v1``) |
...
Options:
  ...
  -o, --output PATH      Output path for the RO-Crate directory. Defaults to
                         output/<name>_crate when --input is given — <name> is
                         the input folder name with a trailing _extracted
  ...
  -I, --interactive      Run in interactive build mode: deterministic pipeline +
      ...
      --smoke-test [MINUTES]
                         ...
                         metadata
  -P, --prompt TEXT      With --interactive --react, an opening instruction that
                         starts the agent working immediately instead of waiting
                         for you to type one. The session stays interactive
                         afterwards. Ignored by the default pipeline build, which
                         runs unprompted
  ...
  -D, --dashboard        Show the profiler dashboard for the latest session (or
                         the one named by --resume)
  -g, --graph            Write the crate's interactive entity explorer and open
                         it. Source is --input (a crate dir or
                         ro-crate-metadata.json) or a session (--resume <id>,
                         else the latest). Needs no LLM config
      --view VIEW        With --graph, which view the explorer opens on:
                         'researcher' (default), 'crate', or 'labprocesses'
                         ('provenance' is the same view under its old name, still
                         accepted). Every view is a toggle inside the page
      --graph-out PATH   Path for the --graph HTML file (default: a temp file)
      --no-browser       With --graph, write the file but do not open a browser
...
> ```bash
> sudo cp /mnt/c/Users/<you>/corp-ca.pem /usr/local/share/ca-certificates/corp-ca.crt
...
### Option C: ``VITRO_OPENAI_CA_BUNDLE`` (LLM endpoint only)

Trust the private CA for the OpenAI-compatible endpoint alone, leaving the rest of
the process on the default trust store:

```bash
export VITRO_OPENAI_CA_BUNDLE=/path/to/your/corp-ca.pem
```

Also settable as ``ca_bundle`` under ``[openai]`` in the config file. A path that
does not exist is rejected with an error rather than silently ignored.
...
| `VITRO_OPENAI_CA_BUNDLE` | No | — | Path to a PEM CA bundle used to verify **only** the OpenAI-compatible endpoint (private / corporate CA). Also settable as `ca_bundle` in `[openai]`. A path that does not exist is an error, not a silent fallback. |
| `VITRO_OPENAI_USE_RESPONSES_API` | No | — (routed by model name) | Force Responses-API routing on/off (`1`/`true`/`yes`/`on`). Needed for custom or Azure deployment names the reasoning-model heuristic cannot recognise. |
| `VITRO_OPENAI_MODEL_PROVIDER` | No | — | Model vendor prefix for **cost display only** (e.g. `deepseek`, `azure`, `openai`, `together`). Also settable as `model_provider` in `[openai]`. |
...
| `VITRO_ANTHROPIC_MODEL_PROVIDER` | No | — | Model vendor prefix for **cost display only**. Also settable as `model_provider` in `[anthropic]`. |
| `VITRO_MAX_RETRIES` | No | `3` | Max retry attempts for LLM API calls (configurable via ``[_global]`` in config file) |
...
| `VITRO_TEMPERATURE` | No | `0` | Sampling temperature, applied identically to both providers and both build arms. A non-numeric value fails loudly rather than resolving to `0`. Never sent to a reasoning model. |
| `VITRO_REQUEST_TIMEOUT` | No | `600` | Per-request wall-clock timeout (seconds) for the chat model. A non-positive or unparseable value falls back to the default — the model is never built without a finite timeout. |
| `VITRO_NO_STREAM` | No | — (off) | `1`/`true`/`yes`/`on` forces response streaming off everywhere. |
| `VITRO_TIMEZONE` | No | `Europe/Amsterdam` | IANA timezone name for timestamps (config file `[display] timezone` is the fallback). An unknown name falls back to UTC. |
| `VITRO_SESSION_DIR` | No | `sessions` | Root directory for per-run session artifacts. |
| `VITRO_VALIDATE_SERIAL` | No | — (off) | `1`/`true`/`yes` forces the SHACL passes to run serially instead of in parallel. |
| `VITRO_NO_PINNED_FOOTER` | No | — (off) | `1`/`true`/`yes` uses the scrolling status bar instead of the pinned terminal footer — the escape-free escape hatch for a terminal that mishandles it. |
| `VITRO_DESCRIBE_EXTERNAL_TERMS` | No | — (off) | Opt in to the self-contained style: describe every externally-referenced vocabulary term as a node in the crate. |
