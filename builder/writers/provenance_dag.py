...
class CategoryStyle(NamedTuple):
    ...
    mermaid: tuple[str, str]
    """The Mermaid node delimiters, e.g. ``("{{", "}}")`` for a hexagon."""
...
# Public: the maturity report both explains this in its legend and tests for it
# to decide whether the explanation is warranted, so all three uses must be the
# same string.
...
def _display_name(node: dict[str, Any]) -> str:
    ...
    metadata, but it does not survive a diagram: the prefix is 16 characters and
    :func:`_svg_trunc` cuts labels at 18, so *every* generated file rendered as
...
def _mermaid_node(mid: str, node: dict[str, Any]) -> str:
    label = f"{_escape(_display_name(node))}<br/><small>{_escape(_tag(node))}</small>"
    _fill, _stroke, open_, close = _mermaid_style(_node_class(node))
...
def render_provenance_mermaid(
    ...
    *,
    ...
    for nid in sorted(mid):
        ...
        by_class.setdefault(_node_class(node), []).append(mid[nid])
    ...
    # Styling — from the shared registry, so a Sample is the same green here as
    # in the crate graph and in the report's diagrams. Only the categories that
    # were actually drawn get a classDef, so the output stays as small as the
    # chain it describes.
    for cls in sorted(by_class):
        fill, stroke, _open, _close = _mermaid_style(cls)
        lines.append(f"  classDef {cls} fill:{fill},stroke:{stroke},color:{_MERMAID_INK};")
...
_SVG_MARK_X = 13  # left inset of a node's secondary mark (person glyph, rules)

# category → (node CSS class, tag CSS class). Generated, so a category added to
# the registry cannot arrive without the classes that draw it — the chemicals,
# cell-line, people and ISA diagrams all reuse this geometry, and every one of
# them used to need a hand-written entry.
_SVG_CLASS = {
    category: (f"n-{category}", f"tag-{category}")
    for category in (*CATEGORY_STYLES, _CTX_CATEGORY)
}
...
def _svg_trunc(text: str, limit: int = 18) -> str:
    ...
    The budget is in *glyphs*, so a variation selector — which selects an emoji's
    presentation and draws nothing itself — is refunded rather than charged. It
    is one code point of zero width, and counting it would cost a real character
    off the end of the name for something invisible.
    """
    text = text.replace("\n", " ").replace("\r", " ").strip()
    budget = limit + text.count(_VARIATION_SELECTOR)
...
def _svg_node_shape(cls: str, x: int, y: int, variant: str = "") -> str:
    """The node's outline path/polygon for category *cls* at ``(x, y)``.

    No two categories share an outline. That is the point of the function: a
    reader who learns "stadium = sample" in one tab must not meet a stadium
    person in the next, and before the registry existed a Person and a Sample
    were the same rounded rectangle in different colours — which fails anyone
    reading in greyscale, in print, or with a colour vision deficiency.

    A "mark" (the container's bars, the file's fold, the person glyph) sits in
    the node's top-left, clear of the centred label: the label is truncated to 18
    glyphs and centred, so it can reach within ~12px of either edge, and a mark
    placed on the label's own line would collide with it.

    ``variant`` appends an extra CSS class to the outline (used by the routed
    diagrams to mark an entity the crate never links to), so state is carried by
    the stylesheet rather than by inline attributes.
    """
    w, h = _SVG_NODE_W, _SVG_NODE_H
    extra = f" {variant}" if variant else ""
    node_cls = _SVG_CLASS.get(cls, _SVG_CLASS[_CTX_CATEGORY])[0]
    box = f'x="{x}" y="{y}" width="{w}" height="{h}"'

    if cls == "process":  # hexagon (chevron ends read as "a step")
        pts = (
            f"{x},{y + h // 2} {x + 13},{y} {x + w - 13},{y} "
            f"{x + w},{y + h // 2} {x + w - 13},{y + h} {x + 13},{y + h}"
        )
        return f'<polygon class="n {node_cls}{extra}" points="{pts}"/>'
    if cls == "protocol":  # trapezoid — an instruction sheet, not a step
        s = 14
        pts = f"{x + s},{y} {x + w - s},{y} {x + w},{y + h} {x},{y + h}"
        return f'<polygon class="n {node_cls}{extra}" points="{pts}"/>'
    if cls == "material":  # stadium (rounded ends) — a sample/material
        return f'<rect class="n {node_cls}{extra}" {box} rx="{h // 2}" ry="{h // 2}"/>'
    if cls == "data":  # document with a folded corner — a File/Table
        ...
        return f'<path class="n {node_cls}{extra}" d="{body}"/><path class="fold" d="{fold}"/>'
    if cls == "container":  # barred block — an ISA container (Investigation/Study/Assay)
        bar = 7
        return (
            f'<rect class="n {node_cls}{extra}" {box} rx="3" ry="3"/>'
            f'<path class="bars" d="M{x + bar},{y} V{y + h} M{x + w - bar},{y} V{y + h}"/>'
        )
    if cls == "agent":  # soft block wearing a person glyph — an individual
        # The head sits at y+10 and the shoulders bottom out at y+18. The label
        # baseline is y+28 at 11.5px, so its cap top is near y+20 — an 18-glyph
        # name centred in 138px reaches back to about x+12 and does pass under
        # the glyph horizontally, which is why the clearance has to be vertical.
        mx, my = x + _SVG_MARK_X, y + 10
        return (
            f'<rect class="n {node_cls}{extra}" {box} rx="8" ry="8"/>'
            f'<path class="figure" d="M{mx},{my - 3.2} a3.2,3.2 0 1,1 -0.01,0 '
            f"M{mx - 5},{my + 8} a5,5 0 0,1 10,0\"/>"
        )
    if cls == "org":  # square-shouldered block — an institution
        return f'<rect class="n {node_cls}{extra}" {box} rx="3" ry="3"/>'
    if cls == "publication":  # block with a masthead rule — an article
        return (
            f'<rect class="n {node_cls}{extra}" {box} rx="3" ry="3"/>'
            f'<path class="rule" d="M{x + _SVG_MARK_X - 5},{y + 11} H{x + w - 8}"/>'
        )
    if cls == "chemical":  # octagon — a compound (distinct from the process hexagon)
        c = 12
        pts = (
        ...
        return f'<polygon class="n {node_cls}{extra}" points="{pts}"/>'
    # annotation, and the ctx fallback — a plain rounded rectangle.
    return f'<rect class="n {node_cls}{extra}" {box} rx="8" ry="8"/>'


# Legend swatch geometry. The node box is 138x48, so the swatch keeps that
# ~2.9:1 aspect and lets the viewBox do the scaling.
_LEGEND_SWATCH_W = 30
_LEGEND_SWATCH_H = 12
_LEGEND_SWATCH_PAD = 7  # room for the stroke, which straddles the outline


def legend_swatch(category: str, variant: str = "") -> str:
    """A miniature of the outline the diagrams actually draw for *category*.

    Drawn by calling the same :func:`_svg_node_shape` the diagram calls and
    letting a ``viewBox`` shrink it, rather than by hand-writing a small
    approximation. Hand-written swatches drift: the report's "File / table" key
    was a plain rounded rectangle for as long as the diagram had been drawing a
    document with a folded corner, so the legend taught readers a shape that
    appeared nowhere in the figure it explained.

    Args:
        category: A key of :data:`CATEGORY_STYLES` (or ``"ctx"``).
        variant: The node state to draw, passed straight through to
            :func:`_svg_node_shape` — so a key for an ``unwired`` node inherits
            the diagram's own dashed styling instead of re-describing it.

    Returns:
        An ``<svg>`` element carrying the ``prov`` class, so it picks up the same
        fill and stroke rules as the diagram it stands for.
    """
    pad = _LEGEND_SWATCH_PAD
    return (
        f'<svg class="prov lg-sw" width="{_LEGEND_SWATCH_W}" height="{_LEGEND_SWATCH_H}" '
        f'viewBox="0 0 {_SVG_NODE_W + pad * 2} {_SVG_NODE_H + pad * 2}" aria-hidden="true">'
        f"{_svg_node_shape(category, pad, pad, variant)}</svg>"
    )
...
def render_provenance_svg(
    ...
    for n in sorted(drawn, key=lambda n: (col[n], row[n], n)):
        ...
        node_svg.append(
            ...
            # never truncated — so the badge abbreviates the label, not the fact.
...
#
# Mermaid needs literal hexes rather than CSS variables, so the fills are mixed
# here from the one registry colour instead of being picked by hand — a
# hand-picked fill is how the crate graph came to disagree with the report.
...
def _tint(colour: str, pct: float, background: tuple[int, int, int] = (255, 255, 255)) -> str:
    """*colour* laid over *background* at *pct* opacity, as a literal hex.

    The CSS views say this with ``color-mix()``; Mermaid has no such function, so
    the same mix is computed here and both come out the same colour.
    """
    channels = (int(colour[1:3], 16), int(colour[3:5], 16), int(colour[5:7], 16))
    return "#%02x%02x%02x" % tuple(
        round(channel * pct + base * (1 - pct)) for channel, base in zip(channels, background)
    )


def category_label(category: str) -> str:
    """The reader-facing name for *category*, shared by every view.

    The overview's cluster headings and its legend read from here together, so a
    key can never name a cluster that the map labels differently.
    """
    style = CATEGORY_STYLES.get(category)
    return style.label if style else "Other"


def _mermaid_style(category: str) -> tuple[str, str, str, str]:
    """``(fill, stroke, shape-open, shape-close)`` for a Mermaid ``classDef``."""
    style = CATEGORY_STYLES.get(category)
    if style is None:
        return (_tint(_CTX_COLOUR, _MERMAID_FILL_PCT), _CTX_COLOUR, "(", ")")
    return (_tint(style.colour, _MERMAID_FILL_PCT), style.colour, *style.mermaid)


_DEFAULT_CATEGORY_STYLE = _mermaid_style(_CTX_CATEGORY)
...
def category_css() -> str:
    """The per-category stylesheet block, generated from the registry.

    CSS cannot loop, so every category used to need three hand-written rules —
    a custom property, an overview tile fill and a node outline — and the ones
    nobody remembered to write were the reason a protocol had a colour in the
    overview and none anywhere else. Generating them means a category cannot
    exist without being drawable.
    ...
    lines.append("}")
    for cat in (*CATEGORY_STYLES, _CTX_CATEGORY):
        var = f"var(--cat-{cat})"
        # Overview tile: fill carries the category, outline sharpens it.
        lines.append(
            f".mat .cat-{cat} {{ fill:color-mix(in srgb,{var} {_TILE_FILL_PCT},var(--surface));"
            f" stroke:{var};"
            f" background:color-mix(in srgb,{var} {_TILE_FILL_PCT},#fff);"
            f" border-color:{var}; }}"
        )
        # Diagram node: a tint of its OWN hue. Before this was generated, a
        # process was filled teal and outlined blue — two different categories'
        # colours on one shape.
        lines.append(
            f".mat .prov .n-{cat} {{"
            f" fill:color-mix(in srgb,{var} {_NODE_FILL_PCT},var(--surface-2));"
            f" stroke:{var}; }}"
        )
        lines.append(f".mat .prov .tag-{cat} {{ fill:{var}; }}")
    return "\n".join(lines)
...
def render_chemicals_svg(inventory: dict[str, Any]) -> str:
    ...
    **Compounds only** (#506). This view used to draw each compound's route —
    ...
    # Peer-aware labels: two compounds must never draw as the same text. Run
    # before drawing so each brief carries the `display` label _svg_place reads.
    ...
    for i, chem in enumerate(ordered):
        ...
        # The inventory's own brief, not a copy: it carries the `display` label
        # just written onto it, and a copy would silently lose it.
        _svg_place(nodes_svg, chem, "chemical", x, y, variant)
...
def render_celllines_svg(inventory: dict[str, Any]) -> str:
    ...
    return _render_routed_svg(
        ...
        # The reader-facing noun is the owner's (review comment); the
        # entities themselves stay cell lines.
        noun="biological samples",
...
def render_isa_svg(inventory: dict[str, Any]) -> str:
    ...
    return _svg_document(
        ...
        # The view's reader-facing name is "Assays" (its tab); the accessible
        # name follows the tab, the LabProcesses one-view-one-name precedent.
        f"Assays: {counts.get('investigations', 0)} investigation, "
...
# That is the registry's own order, and taking it from there means a new
# category appears in the map instead of being silently dropped — which is what
# happened to organisations when they stopped being filed under "agent".
_OV_CATEGORY_ORDER: tuple[str, ...] = (*CATEGORY_STYLES, _CTX_CATEGORY)
...
def _crate_node_shape(mid: str, n: dict[str, Any]) -> str:
    ...
    _f, _s, open_, close = _mermaid_style(n["category"])
    return f'{mid}{open_}"{label}"{close}'
...
def render_crate_graph(
    ...
    *,
    ...
    for cat in CATEGORY_STYLES:
        fill, stroke, _o, _c = _mermaid_style(cat)
        lines.append(f"  classDef cat_{cat} fill:{fill},stroke:{stroke},color:{_MERMAID_INK};")
...
# Off-graph reference stubs. Not entity categories — a status, not a kind — so
# they are listed here rather than in the registry, and keep the parallelogram
# that has always said "this is a pointer, not a thing".
_LEGEND_STATUS_ITEMS: tuple[tuple[str, str, str], ...] = (
...
def _crate_legend_lines() -> list[str]:
    ...
    Generated from the registry, in registry order, so it lists every category
    that can be drawn and none that cannot. The hand-written list it replaced had
    fallen behind: it offered a single "Person / Org" swatch for two categories
    the diagram draws differently, and named neither publications nor
    organisations at all.
    """
    lines = ['  subgraph legend_g["Legend — node colour = entity type · box = layer"]']
    lines.append("    direction LR")
    for cat, style in CATEGORY_STYLES.items():
        open_, close = style.mermaid
        lines.append(f'    lg_{cat}{open_}"{style.label}"{close}:::cat_{cat}')
    for mid_, label, cls in _LEGEND_STATUS_ITEMS:
