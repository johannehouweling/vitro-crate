...
def _context_terms() -> frozenset[str]:
    """Every property name the crate's JSON-LD context defines."""
    terms: set[str] = set()
...
def _scalar_props(entity: Entity, skip: tuple[str, ...] = ()) -> dict[str, Any]:
    ...
    A snake_case key that the ISA-Tox ``@context`` does NOT define is dropped.
    Emitting one produced a bare JSON-LD key absent from the context, which
    fails BASE with "not allowed in the compacted JSON-LD context" — and the
    agent cannot fix that by editing the crate, because the key is regenerated
    from state on every build (observed: ``release_date`` and
    ``measurement_method`` written alongside the valid ``releaseDate`` /
    ``measurementMethod``).

    Membership is checked against the context, NOT guessed from the spelling:
    the AOP ontology's terms are genuinely snake_case (``has_key_event``,
    ``upstream_event``, ``short_name``, …) and dropping those would quietly
    dismantle the AOP subgraph. camelCase keys pass through untouched — the
    RO-Crate base context resolves the schema.org vocabulary, which this module
    cannot enumerate.
...
def _build_csvw_schema(
    ...
    *,
    ...
    for col in columns:
        title = col["titles"]
        # `name` alongside `titles`. csvw:title is the CSVW way to label a column
        # and schema:name is the RO-Crate way; a column carrying only the former
        # tripped BOTH the "contextual entity MUST have a name" and "entities
        # SHOULD have a human-readable name" checks — 26 findings from 13
        # columns that were already labelled. Nothing new is asserted: the title
    ...
    *,
    ...
    return crate.add(
        File(
            ...
            source,
            dest_path=rel,
            properties={"@type": ["File", "csvw:Table"], "name": _autogenerated_name(name)},
        )
    )
