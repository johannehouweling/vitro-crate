...
_STRUCT_FIELDS = frozenset(
    {
        ...
        "path",
        "contentUrl",
        # Placeholder marker (#438) — consumed to materialise the empty
        # stand-in, never emitted as a stray literal on the File node.
        "provisional",
...
def _known_file_size(state: CrateState, fe: Entity, input_path: str | None) -> int | None:
    ...
    # A placeholder for an output nobody deposited is empty, on both paths. It
    # used to be sized from the header line the build was about to write; there
    # is no header now (#589), and an empty file's size is a fact rather than a
    # prediction — so the in-memory and the written crate still agree.
...
def _add_leaves(
    ...
    *,
    ...
    for fe in state.list_entities("File"):
        ...
        # contain (#438). Materialise an empty one instead.
        if source is None and materialize_payload and fe.fields.get("provisional"):
            source = _materialize_missing_output(fe, output_dir, _file_dest(fe))
        ...
        if fe.fields.get("provisional"):
            ...
            # eye actually show — and a stand-in the tool minted to satisfy a
            ...
            props["description"] = _MISSING_OUTPUT_NOTE
        _idx_add(
            idx,
            fe,
            crate.add(
...
# A step whose output the deposit does not contain. The file exists so the crate
# does not reference something it never shipped (#438) and so the chain reaches a
# terminus, but it is EMPTY and says so.
#
# It used to be a csvw:Table with a typed schema over columns read from a module
# constant — the same three or four headers in every crate, telling a reader
# nothing about this experiment while costing 110 column entities in one real
# build (#589). Declaring a shape for a file nobody produced is a claim about
# data that does not exist; the honest statement is that the output is missing.
_MISSING_OUTPUT_NOTE = (
    "No output file for this step was found in the deposit. This placeholder "
    "keeps the derivation chain complete and contains NO data — replace it with "
    "the step's real output, or record why none exists."
)


def _materialize_missing_output(fe: Entity, output_dir: Path | None, rel: str) -> str | None:
    """Write the empty stand-in for an undeposited output; return its path.

    Returns ``None`` when there is nothing to write (not a placeholder, or the
    in-memory validate path where no payload is materialised), leaving the
    caller's source resolution untouched.

    The file is created empty. It previously carried a header line naming the
    columns the step "should" have produced, which read as a template a curator
    could fill in — but those column names were a constant, not a reading of this
    experiment, and over zero rows the CSVW schema built from them made a
    vacuously-true conformance claim (#473, #589).
    """
