...
def _populate_root_and_conformance(state: CrateState, crate: ROCrate) -> None:
    ...
    # The DOI the deposit declares outranks its accession: both identify the deposit,
    # but only the DOI still does so outside the repository it came from, which is the
    # whole of what DSM-1-C0 and RDA-F1-01M ask (#682).
    if m.doi:
        crate.root_dataset["identifier"] = m.doi
    elif m.accession:
