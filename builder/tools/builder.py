...
def assemble_crate(
    ...
    *,
    ...
    # Both of these read the FINISHED graph, so they run last: one describes the
    # ontology terms the crate points at, the other gives every domain-typed node
    # a schema.org type a generic consumer can act on.
...
# A schema.org type to carry ALONGSIDE each non-schema.org domain type. Written
# as `schema:` CURIEs on purpose: the crate context maps the bare token
# "BioChemEntity" to bioschemas.org, so an unprefixed name would re-state the
# same non-schema.org namespace the check is complaining about.
#
# Each pairing is a claim the crate already makes elsewhere, not a new one:
#   LabProcess    the context notes "schema:object is the real schema.org
#                 Action/LabProcess input predicate" and the build emits
#                 object/result — those are schema:Action properties, and a
#                 process that produces its results is a CreateAction.
#   LabProtocol   Bioschemas' lab-protocol profile is built on schema:HowTo.
#   Sample /      schema.org's own MolecularEntity sits under BioChemEntity;
#   MolecularEntity   a biological sample is the same kind of thing.
#   csvw:Column   the one judgement call here — a column carries a name/titles
#                 and a propertyUrl pointing at an ontology term, which is what
#                 a DefinedTerm is. Least certain of the set; easy to drop.
_SCHEMA_ORG_COMPANION_TYPES: dict[str, str] = {
    "LabProcess": "schema:CreateAction",
    "LabProtocol": "schema:HowTo",
    "Sample": "schema:BioChemEntity",
    "CellLineSample": "schema:BioChemEntity",
    "MolecularEntity": "schema:BioChemEntity",
    "csvw:Column": "schema:DefinedTerm",
}


def add_schema_org_types(crate: ROCrate) -> int:
    """Give every Bioschemas/CSVW-typed node a schema.org type as well.

    RO-Crate 1.2 RECOMMENDS that each entity carry at least one type in the
    schema.org namespace, and the shape enforcing it is purely syntactic: it
    looks for a type IRI starting with ``http(s)://schema.org/``. Our domain
    types resolve to ``https://bioschemas.org/…``, so a LabProcess, a Sample and
    a compound each failed the check while being perfectly well typed.

    The domain type stays FIRST and is never replaced — it is the specific,
    meaningful one, and the schema.org term is the general one a generic
    consumer can understand. Nodes that already carry a schema.org type are left
    ...
    for entity in list(crate.get_entities()):
        ...
        if any(t in _SCHEMA_ORG_TYPE_TOKENS or t.startswith("schema:") for t in types):
            continue
        companion = next(
            (_SCHEMA_ORG_COMPANION_TYPES[t] for t in types if t in _SCHEMA_ORG_COMPANION_TYPES),
            None,
        )
        if companion is None or companion in types:
            continue
        # ro-crate-py refuses `entity["@type"] = …` (it guards the reserved keys),
        # so write the backing JSON-LD directly — the only route it offers for
        # adding a type to an existing entity.
        entity._jsonld["@type"] = types + [companion]
        updated += 1
    if updated:
        logger.info("Added a schema.org companion type to %d entit(ies)", updated)
    return updated
