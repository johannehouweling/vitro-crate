...
TOOL_SPECS = [
    ...
    {
        "name": "export_crate",
        "description": "Write the finished RO-Crate to disk (the only step that touches disk). Call it ONCE, when your work on the crate is FINISHED — not when REQUIRED turns green. Conformance is not completion: 'ok' here means the REQUIRED gate passed, while recommended findings, unanswered questions and unwired entities are all still work. Use build_and_validate to check your progress; it assembles in memory, touches no disk, and answers the same question. Every mid-loop export writes a complete copy of the crate for output nobody reads until the end. Returns crate_path. Auto-embeds the browsable preview and the entity-graph diagram (ro-crate-graph.mmd, a CreativeWork about ./) so the crate is self-describing. Before writing, it validates at the FULL gate (REQUIRED + RECOMMENDED + OPTIONAL) so the embedded maturity report covers every tier, and returns 'validation' with per-tier issue_counts; 'ok' there is REQUIRED conformance only, so RECOMMENDED/OPTIONAL counts are findings to report, not a failed export. When output_path is omitted, defaults to sessions/<session_id>/working_crate/",
        "parameters": {
            "type": "object",
            "properties": {
                "output_path": {
