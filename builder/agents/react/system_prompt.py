...
- materialize_aop_subgraph: Turn one AOP-Wiki id into the full subgraph (AdverseOutcomePathway + KeyEvents + KeyEventRelationships, cross-linked) wired onto a Study: the one study_id names, or the sole Study when you pass none (with several, name one or the pathway stays detached)
...
- draft_file: Create a File data entity (raw measurements, processed results, figures); pass programming_language for an analysis script
