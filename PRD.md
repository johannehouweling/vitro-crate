...
The system:
1. Scans input (files, BioStudies JSON, conversation, or existing crate) to classify files and seed an initial state
...
## User Stories
1. As a toxicology researcher, I want to upload a BioStudies JSON file, so that the builder seeds the crate with existing metadata rather than starting from scratch.
2. As a toxicology researcher, I want to point the builder at a raw data directory, so that files are classified and I can describe the experiment conversationally.
...
10. As a toxicology researcher, I want to review the agent's file classifications before drafting begins, so that I can correct mismatches.
11. As a toxicology researcher, I want to review each drafted entity before it's committed, so that I can approve, edit, or reject the agent's work.
12. As a toxicology researcher, I want to see a validation report after drafting, so that I know which REQUIRED fields are still missing and what SHOULD/MAY improvements are available.
13. As a toxicology researcher, I want the builder to block crate assembly if there are any REQUIRED validation failures, so that I never produce an invalid RO-Crate.
14. As a toxicology researcher, I want to see MIT coverage scores per module (Chemical Information, Biological Model Information, General Information), so that I can prioritize filling gaps.
15. As a toxicology researcher, I want to see FAIR maturity scores, so that I understand how findable, accessible, interoperable, and reusable my crate is.
16. As a toxicology researcher, I want to save my session mid-way, so that I can resume work later without losing progress.
17. As a toxicology researcher, I want the builder to auto-save after key milestones (scan, draft, validate, HITL), so that I never lose work.
18. As a toxicology researcher, I want to resume a saved session, so that the agent picks up from its last checkpoint with full context.
19. As a toxicology researcher, I want to run validation on the assembled crate before finalization, so that I catch any issues introduced during assembly.
20. As a toxicology researcher, I want the agent to detect when it's stuck and escalate to me for guidance.
21. As a toxicology researcher, I want to optionally produce an ARC folder alongside the RO-Crate, so that I can work with both formats.
22. As a developer integrating this builder, I want to understand the toolbox interface, so that I can add new tools or input readers without changing the agent loop.
23. As a developer extending the system, I want field-level completion metadata on every entity, so that I can build custom UIs that show exactly what's missing.
...
Each input format (BioStudies JSON, ARC folder, existing crate, directory, conversation) has a dedicated reader that converts to canonical entity state in `CrateState`. The core agent loop never touches input formats directly.
...
- `input/metadata/` — Example BioStudies JSON inputs (S-VHPS21, S-VHPS22, S-VHPS26)
...
4. **Input readers** — Each reader (BioStudies JSON, directory scanner) given example input -> correct CrateState seed.
...
- **ARC workflow integration**: ARC output is optional. The builder does not require an ARC folder structure to operate.
