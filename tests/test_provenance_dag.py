...
class TestSharedSelectors:
    ...
    def test_the_chain_svg_draws_exactly_the_derivation_endpoints(self) -> None:
        ...
        members = next(
            v["members"] for v in build_explorer_payload(graph)["views"] if v["key"] == "processes"
        )
...
class TestBuildChemicalInventory:
    ...
    def test_wired_route_names_the_producing_process(self) -> None:
        route = self._by_name(build_chemical_inventory(_chemicals_graph()))["Aflatoxin B1"]["route"]
    ...
    def test_valueurl_route_walks_the_csvw_containment_chain(self) -> None:
        ...
        route = self._by_name(build_chemical_inventory(_chemicals_graph()))["Bisphenol A"]["route"]
        assert route["process"] == "#exposure"
        assert route["via"] == "#table"
...
class TestMalformedGraphDegradesGracefully:
    ...
    def test_non_string_id_is_skipped_not_fatal(self) -> None:
        ...
        assert [c["name"] for c in build_chemical_inventory(graph)["chemicals"]] == ["Aflatoxin B1"]
        assert build_cellline_inventory(graph)["celllines"] == []
    ...
    def _graph(self) -> dict:
        return {
            "@graph": [
                ...
                *[{"@id": f"#p{i}", "@type": "Person", "name": "Same Name"} for i in range(6)],
                *[
...
class TestCategoryRegistry:
    ...
    def test_every_category_has_a_unique_glyph(self) -> None:
        ...
        deficiency, so two categories sharing a glyph would leave colour as the
        only thing telling them apart.

        Nothing draws these since #688 chose colour only; the rule is held anyway,
        because the data is kept so that restoring the channel stays a one-line
        change, and a registry that quietly grew a duplicate in the meantime
        would not be restorable."""
